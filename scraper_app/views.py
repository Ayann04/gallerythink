...
def _screenshot_page(driver: webdriver.Chrome) -> Image.Image:
    """Capture full page screenshot as a PIL Image."""
    png = driver.get_screenshot_as_png()
...
def trigger_scrape(request):
    ...
    try:
        ...
        try:
            ...
            time.sleep(5)
            search_button = driver.find_elements(By.XPATH,"//button[contains(., 'Search')]")
            search_button[1].click() 
            time.sleep(50)
            _create_status(new_run, "Screenshot after search click", pil_image=_screenshot_page(driver))
        ...
        try:
            while True:  # Keep looping through all pages until no next button
                    _create_status(new_run, "Fetch all record links on current page", pil_image=_screenshot_page(driver))
